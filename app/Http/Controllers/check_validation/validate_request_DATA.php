 ...
 */
    ...
    foreach($mayBe_params_val as $item){
        ...
        if(isset($params_types_Arr[$key])){
            ...
            if(int_types_MySQL($type_data) && (!is_numeric($item))){
                http_response_code(400);
                $mess .= 'Неверный тип параметра запроса: '. $key. ' = '. $item .' (' . gettype($item). '), тогда как требуется целый тип.';
            }
// Остальные типы переменных (например, float) из запроса нужно анализировать дополнительно! +++

            if(strlen($item) > $max_len_data){
 ...
 */
    ...
    foreach ($mayBe_params as $field){
        if(!array_key_exists($field, $public_request_validate)){
            ...
            throw new ErrorException('Предполагаемое поле базы данных из запроса НЕ ЗАДАНО в белом списке: '. $field);
        }else{
